...
namespace Test;
use Test\JsonWebTokenInterface;
...
class Token implements JsonWebTokenInterface
{
    ...
    {
        $header = $this->header;
        $header["end"] = $this->setEndToken($lifeSecond);
        $algoritm = $header['alg'];
        $secret = $this->secret;
    ...
    public function verify(string $token): bool
    {
        ...
        $signature2 = hash_hmac($algoritm, $signature2, $secret);
        $signature2 = base64_encode($signature2);

        $header1 = base64_decode($header);
        $header1 = json_decode($header1,true);
        $end = $header1['end'];
